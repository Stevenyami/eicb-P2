...
/** A combined identification and type checking visitor. */
	...
	public Type visitVariableAssignment(VariableAssignment variableAssignment, Void __) {
		Type ident = variableAssignment.identifier.accept(this);// hole der ident und akzeptiert det Type
		Type value = variableAssignment.value.accept(this); // hole die Expr der Value und akzeptiert der Type
		checkType(variableAssignment,ident, value );
		return null;
	}
	
	@Override
	public Type visitLeftHandIdentifier(LeftHandIdentifier leftHandIdentifier, Void __) {
		Declaration decl = table.getDeclaration(leftHandIdentifier.name);// hole die deklaration
		if(decl.isVariable()){  // prüft, ob es eine Variable ist
	...
	public Type visitMatrixLhsIdentifier(MatrixLhsIdentifier matrixLhsIdentifier, Void __) {
		Declaration dec = table.getDeclaration(matrixLhsIdentifier.name);
		Type x = matrixLhsIdentifier.rowIndexExpression.accept(this); // hole die Anzahl von Rows
		Type y = matrixLhsIdentifier.colIndexExpression.accept(this); // hole die Anzahl von Cols

		if(!dec.isVariable()){
			throw new ConstantAssignmentError(matrixLhsIdentifier, dec);
		}
		if (!(dec.getType() instanceof MatrixType)){
			throw new InapplicableOperationError(matrixLhsIdentifier, dec.getType(), MatrixType.class);
		}
		checkType(matrixLhsIdentifier, x, IntType.instance); // mit Hilde der CheckType() methode, ob x und y integer sind
		checkType(matrixLhsIdentifier, y , IntType.instance);
		matrixLhsIdentifier.setDeclaration(table.getDeclaration(matrixLhsIdentifier.name));

		return ((MatrixType) dec.getType()).elementType;
	}
	
	@Override
	public Type visitVectorLhsIdentifier(VectorLhsIdentifier vectorLhsIdentifier, Void __) {
		// ZB : a[15] = 4 || b[3] = 1.5
		Declaration dec = table.getDeclaration(vectorLhsIdentifier.name);
		if(!dec.isVariable()){
			throw new ConstantAssignmentError(vectorLhsIdentifier, dec);
		}
		if (!(dec.getType() instanceof VectorType)){ //  Vektor ??
			throw new InapplicableOperationError(vectorLhsIdentifier, dec.getType(), VectorType.class);
		}
		Type x = vectorLhsIdentifier.indexExpression.accept(this); // hole die Expression des Index
		checkType(vectorLhsIdentifier, x, IntType.instance); // prüft, ob x Integer ist, sons wirf einen Fehler
		vectorLhsIdentifier.setDeclaration(dec);
		return ((VectorType) dec.getType()).elementType;
	}
	
	@Override
	public Type visitRecordLhsIdentifier(RecordLhsIdentifier recordLhsIdentifier, Void __) {
		// ZB: x@r = 3 // x@r = 3.4
		Declaration dec = table.getDeclaration(recordLhsIdentifier.name);
		if(!dec.isVariable()){
			throw new ConstantAssignmentError(recordLhsIdentifier, dec);
		}
		if (!(dec.getType()instanceof RecordType)) {
            throw new InapplicableOperationError(recordLhsIdentifier, dec.getType(), RecordType.class);
        }
        RecordElementDeclaration r =((RecordType) dec.getType()).typeDeclaration.getElement(recordLhsIdentifier.elementName);
        if(r == null){
            throw new RecordElementError(recordLhsIdentifier, recordLhsIdentifier.name, recordLhsIdentifier.elementName);
        }
        if (!r.isVariable()){
            throw new ConstantAssignmentError(recordLhsIdentifier, r);
        }
		recordLhsIdentifier.setDeclaration(dec);
		return r.getType();
	}
	
	@Override
	public Type visitForLoop(ForLoop forLoop, Void __) {
		// for loop in MAVL::= for (counter = 0; counter < 10; counter = counter + 1){ i = i+1; }
        table.openNewScope();
        // Hole die Declaration , Expr akzeptieren und declaration setzen
		Declaration initVAr = table.getDeclaration(forLoop.initVarName);
        forLoop.setInitVarDeclaration(initVAr);
		Expression iniVarExpr = forLoop.initExpression;
		iniVarExpr.accept(this);
        Declaration incrVar = table.getDeclaration(forLoop.incrVarName);
        forLoop.setIncrVarDeclaration(incrVar);
        Expression incrVarExp = forLoop.incrExpression;
        incrVarExp.accept(this);

        checkType(forLoop, initVAr.getType(), iniVarExpr.getType());
        checkType(forLoop, incrVar.getType(), incrVarExp.getType());
        //hole Expr cond
		Expression cond = forLoop.loopCondition;
		cond.accept(this);
        checkType(forLoop, cond.getType(), BoolType.instance);

		if (!initVAr.isVariable()){
			throw new ConstantAssignmentError(forLoop, initVAr);
		}
		if (!incrVar.isVariable()){
			throw new ConstantAssignmentError(forLoop, incrVar);
		}
		table.openNewScope();
	...
	public Type visitCallStatement(CallStatement callStatement, Void __) {
		callStatement.callExpression.accept(this); // Aufruf von dem ExprStatement anhand der Attribute callEXpr.
		return null;
	}
	...
	public Type visitCompoundStatement(CompoundStatement compoundStatement, Void __) {
		table.openNewScope();
		for(Statement statement : compoundStatement.statements){ // durchlauf einfach alle Statement
			statement.accept(this);
		}
		table.closeCurrentScope();
		return null;
	...
	public Type visitMatrixMultiplication(MatrixMultiplication matrixMultiplication, Void __) {
		Type leftOp = matrixMultiplication.leftOperand.accept(this);// Hole die beiden Operanden und akzeptiere die Expression
		...
		MatrixType leftM = (MatrixType) leftOp; // casten die Operanden in Matrix
		MatrixType rightM = (MatrixType) rightOP;
		checkType(matrixMultiplication, leftM.elementType, rightM.elementType); // und checkt anhand der Attribute elementType von der Klasse StrukturType, ob sie beide gleiche Type haben

		if(leftM.rows != rightM.cols){
			throw new StructureDimensionError(matrixMultiplication, leftM.rows, rightM.cols);
		}
		MatrixType result = new MatrixType(leftM.elementType, leftM.rows, rightM.cols);
		matrixMultiplication.setType(result);
		return result;
	}
	...
	public Type visitCompare(Compare compare, Void __) {
		Type OpLeft = compare.leftOperand.accept(this); // Holle die beiden Operanden, die verglichen werden müssen
		Type OpRight = compare.rightOperand.accept(this);
		if (!OpLeft.isNumericType()){
			throw new InapplicableOperationError(compare, OpLeft, NumericType.class);
		}
		if (!OpRight.isNumericType()){
			throw new InapplicableOperationError(compare, OpRight, NumericType.class);
		}
		checkType(compare, OpLeft, OpRight); // Prüfung, ob beide gleiche Type sind
	...
	public Type visitMatrixTranspose(MatrixTranspose matrixTranspose, Void __) {
		Type OpM = matrixTranspose.operand.accept(this); // hole type der Operand
		if (!(OpM instanceof MatrixType)){
			throw new InapplicableOperationError(matrixTranspose,OpM, MatrixType.class);
		}
		MatrixType newMatrix = (MatrixType) OpM;
		MatrixType result = new MatrixType(newMatrix.elementType, newMatrix.cols, newMatrix.rows);
		matrixTranspose.setType(result);
		return result;
	}
	...
	public Type visitCallExpression(CallExpression callExpression, Void __) {
		Function f = env.getFunctionDeclaration(callExpression.functionName); // hole dec der Function und dannach setzen
        callExpression.setCalleeDefinition(f);
        if (f.parameters.size() != callExpression.actualParameters.size()){ // Prüft, ob gleiche Anzahl an elemente
            throw new ArgumentCountError(callExpression,f, f.parameters.size(), callExpression.actualParameters.size());
        }
        for(int i = 0 ; i < callExpression.actualParameters.size(); i++){
            checkType(callExpression, callExpression.actualParameters.get(i).accept(this), f.parameters.get(i).typeSpecifier.getType());
        }
        callExpression.setType(f.returnTypeSpecifier.getType());
        f.setReturnType(f.returnTypeSpecifier.getType());
	...
	public Type visitRecordInit(RecordInit recordInit, Void __) {
		...
		//val int i;}
		RecordTypeDeclaration recordDecl = env.getRecordTypeDeclaration(recordInit.typeName);
        if (recordInit.elements.size() != recordDecl.elements.size()){
            throw new StructureDimensionError(recordInit, recordInit.elements.size(), recordDecl.elements.size());
        }
        for(int i = 0; i < recordDecl.elements.size(); i++){ // überprüfen der Type von jeder Expression, ob es übereinstimmt mit der Type der RecordInit
            checkType(recordInit, recordDecl.elements.get(i).getType(), recordInit.elements.get(i).accept(this));
        }
        recordInit.setType(new RecordType(recordInit.typeName, recordDecl));
